...
static SD_Error_t SD_DoInit(void)
{
    ...
    if (card.sd_card_information.flags & SD_SupportHighCapacityFlag) {
        SD_CardType = SD_HIGH_CAPACITY;
    } else {
        SD_CardType = SD_STD_CAPACITY_V2_0;
    }

    // STATIC_ASSERT(sizeof(SD_Handle.CSD) == sizeof(card.CSD), hal-csd-size-error);
    // memcpy(&SD_Handle.CSD, &card.CSD, sizeof(SD_Handle.CSD));

    // STATIC_ASSERT(sizeof(SD_Handle.CID) == sizeof(card.CID), hal-cid-size-error);
    // memcpy(&SD_Handle.CID, &card.CID, sizeof(SD_Handle.CID));
...
SD_Error_t SD_Erase(uint64_t StartAddress, uint64_t EndAddress)
{
    ...
    if (SD_Erase_Block(&card, startBlock, blockCount) != Status_Success) {
        ErrorState = SD_ERROR;
    }
